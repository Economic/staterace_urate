...
current_urates <- current_quarter_output %>% 
  filter(gender=='All') %>% 
  pivot_wider(id_cols=state, names_from = wbhao, values_from = suppressed_urates, names_prefix = 'c_')
  
reference_urates <- reference_quarter_output %>% 
  filter(gender=='All') %>% 
  pivot_wider(id_cols=state, names_from = wbhao, values_from = suppressed_urates, names_prefix = 'r_')

#Uemployment ratio tables
current_qtr_ratios_table <- current_urates %>% 
  mutate(c_bw_ratio = c_Black/c_White,
         c_hw_ratio = c_Hispanic/c_White) %>% 
  select(state, c_bw_ratio, c_hw_ratio)

reference_qtr_ratios_table <- reference_urates %>% 
  mutate(r_bw_ratio = r_Black/r_White,
         r_hw_ratio = r_Hispanic/r_White) %>% 
...
change_since_previous_period <- current_urates %>% 
  ...
  mutate(d_All = (c_All - r_All)*100,
         d_White = (c_White - r_White)*100,
         d_Black = (c_Black - r_Black)*100,
         d_Hispanic = (c_Hispanic - r_Hispanic)*100,
         d_AAPI = (c_Asian - r_Asian)*100) %>% 
...
addWorksheet(EARN_staterace_umemp, sheetName = "2021 Q4 raw")
addWorksheet(EARN_staterace_umemp, sheetName = "Current Urates")
addWorksheet(EARN_staterace_umemp, sheetName = "Reference Urates")
addWorksheet(EARN_staterace_umemp, sheetName = "Current BW HW ratios")
addWorksheet(EARN_staterace_umemp, sheetName = "Reference BW HW ratios")
addWorksheet(EARN_staterace_umemp, sheetName = "Change between periods")
...
writeData(EARN_staterace_umemp, headerStyle = hs1, current_quarter_output, sheet = "2021 Q4 raw",
          startCol = 1, startRow = 1, colNames = TRUE, keepNA = TRUE)
writeData(EARN_staterace_umemp, headerStyle = hs1, reference_quarter_output, sheet = "2020 Q1 raw",
          startCol = 1, startRow = 1, colNames = TRUE, keepNA = TRUE)
writeData(EARN_staterace_umemp, headerStyle = hs1, current_urates, sheet = "Current Urates",
          startCol = 1, startRow = 1, colNames = TRUE, keepNA = TRUE)
writeData(EARN_staterace_umemp, headerStyle = hs1, reference_urates, sheet = "Reference Urates",
          startCol = 1, startRow = 1, colNames = TRUE, keepNA = TRUE)
writeData(EARN_staterace_umemp, headerStyle = hs1, current_qtr_ratios_table, sheet = "Current BW HW ratios",
          startCol = 1, startRow = 1, colNames = TRUE, keepNA = TRUE)
writeData(EARN_staterace_umemp, headerStyle = hs1, reference_qtr_ratios_table , sheet = "Reference BW HW ratios",
          startCol = 1, startRow = 1, colNames = TRUE, keepNA = TRUE)
writeData(EARN_staterace_umemp, headerStyle = hs1, change_since_previous_period , sheet = "Change between periods",
          startCol = 1, startRow = 1, colNames = TRUE, keepNA = TRUE)
...
#add ppt format
addStyle(EARN_staterace_umemp, "Current BW HW ratios", style=ppt, cols=c(2:3), rows=2:(nrow(current_qtr_ratios_table)+1), gridExpand=TRUE)
addStyle(EARN_staterace_umemp, "Reference BW HW ratios", style=ppt, cols=c(2:3), rows=2:(nrow(reference_qtr_ratios_table)+1), gridExpand=TRUE)
addStyle(EARN_staterace_umemp, "Change between periods", style=ppt, cols=c(2:6), rows=2:(nrow(change_since_previous_period)+1), gridExpand=TRUE)

#add pct format
addStyle(EARN_staterace_umemp, "Current Urates", style=pct, cols=c(2:6), rows=2:(nrow(current_urates)+1), gridExpand=TRUE)
addStyle(EARN_staterace_umemp, "Reference Urates", style=pct, cols=c(2:6), rows=2:(nrow(reference_urates)+1), gridExpand=TRUE)
...
#set column widths
setColWidths(EARN_staterace_umemp, "Current Urates", cols = 2:ncol(current_urates), widths = "auto")
setColWidths(EARN_staterace_umemp, "Reference Urates", cols = 2:ncol(reference_urates), widths = "auto")
setColWidths(EARN_staterace_umemp, "Current BW HW ratios", cols = 2:ncol(current_qtr_ratios_table), widths = "auto")
setColWidths(EARN_staterace_umemp, "Reference BW HW ratios", cols = 2:ncol(reference_qtr_ratios_table), widths = "auto")
setColWidths(EARN_staterace_umemp, "Change between periods", cols = 2:ncol(change_since_previous_period), widths = "auto")
...
saveWorkbook(EARN_staterace_umemp, here("data/EARN_staterace_2021Q4.xlsx"), overwrite = TRUE)
```
