...
cps <-load_basic(2016:2021, age, female, wbhao, statefips, unemp, month, basicwgt, lfstat, age, year) %>% 
  ...
  mutate(qtr=as.yearqtr(date),
         qtr2 = roll) %>%  #create quarter variable
...
all <- cps %>% 
  ...
  summarize(urate = weighted.mean(unemp, w=basicwgt/3, na.rm=TRUE),
            n=n()) %>%
  ungroup()# %>% 
  mutate(roll_urate = rollmean(urate, k=2, fill=NA, align = "right"),
         roll_n = rollmean(n, k=2, fill=NA, align='right'))# %>% 
  pivot_wider(id_cols=statefips, values_from=c(rolling_avg), names_from=c(qtr)) %>% 
...
race <- cps %>% 
  ...
  summarize(urate = weighted.mean(unemp, w=basicwgt/3, na.rm=TRUE),
            n=n()) %>% 
  pivot_wider(id_cols=wbhao, values_from=urate, names_from=qtr)
...
all_subgroups <- cps %>% 
  ...
  summarize(cps_urate = weighted.mean(unemp, w=basicwgt/3, na.rm=TRUE),
            n=n()) %>% 
